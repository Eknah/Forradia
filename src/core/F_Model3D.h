...
#include "F_Math3D.h"
#include "F_Mesh.h"
#include "F_AlgorithmsVectors.h"
#include "F_AlgorithmsStrings.h"

namespace Forradia
{

class F_Model3D : public F_VectorAlgorithms, F_AlgorithmsStrings
{
...
public:
    ...
    inline bool LoadFile(std::string Path)
    {
        ...
        std::vector<F_Vector3> Positions;
        std::vector<F_Vector2> TCoords;
        std::vector<F_Vector3> Normals;
        ...
        while (std::getline(file, curline))
        {
            if (FirstToken(curline) == "o" || FirstToken(curline) == "g" || curline[0] == 'g')
            {
                ...
                else
                {
                    ...
                    {
                        tempMesh = F_Mesh(Vertices, Indices);
    ...
    std::vector<F_Mesh> LoadedMeshes;
    std::vector<F_Vertex> LoadedVertices;
    std::vector<unsigned int> LoadedIndices;
    std::vector<F_Material> LoadedMaterials;

private:

    inline void GenVerticesFromRawOBJ(std::vector<F_Vertex>& oVerts, const std::vector<F_Vector3>& iPositions, const std::vector<F_Vector2>& iTCoords, const std::vector<F_Vector3>& iNormals, std::string icurline)
    {
        ...
        for (int i = 0; i < int(sface.size()); i++)
        {
            ...
            case 1:
            {
                ...
                vVert.TextureCoordinate = F_Vector2(0, 0);
                noNormal = true;
        ...
        if (noNormal)
        {
            F_Vector3 A = oVerts[0].Position - oVerts[1].Position;
            F_Vector3 B = oVerts[2].Position - oVerts[1].Position;

            F_Vector3 Normal = Math3D.Cross(A, B);

            for (int i = 0; i < int(oVerts.size()); i++)
                oVerts[i].Normal = Normal;
        }
    }
    inline void VertexTriangluation(std::vector<unsigned int>& oIndices, const std::vector<F_Vertex>& iVerts)
    {
        ...
        std::vector<F_Vertex> tVerts = iVerts;
