...
void cGameWorldRenderer::Render()
{
    ...
    auto OffsetX = -(float)(2.0f * Camera.GetRenderDistance() + 1.0f) / 2.0f * Engine.TileSize - SubStepX * Engine.TileSize;
    auto OffsetY = -(float)(2.0f * Camera.GetRenderDistance() - 1.0f) / 2.0f * Engine.TileSize - SubStepY * Engine.TileSize;
    ...
    auto ElevPlayer0 = Engine.GetCurrentMapArea().Tiles[PlayerXInt][PlayerYInt].Elevation / ElevAmount;
    auto ElevPlayer1 = Engine.GetCurrentMapArea().Tiles[PlayerXInt + 1][PlayerYInt].Elevation / ElevAmount;
    auto ElevPlayer2 = Engine.GetCurrentMapArea().Tiles[PlayerXInt + 1][PlayerYInt + 1].Elevation / ElevAmount;
    auto ElevPlayer3 = Engine.GetCurrentMapArea().Tiles[PlayerXInt][PlayerYInt + 1].Elevation / ElevAmount;
    auto ElevX = Engine.Player.Position.X - PlayerXInt;
    auto ElevY = Engine.Player.Position.Y - PlayerYInt;
    auto ElevPlayer = (ElevPlayer0 + (ElevPlayer1 - ElevPlayer0) * ElevX + ElevPlayer3 + (ElevPlayer2 - ElevPlayer3) * ElevX + ElevPlayer0 + (ElevPlayer3 - ElevPlayer0) * ElevY + ElevPlayer1 + (ElevPlayer2 - ElevPlayer1) * ElevY) / 4.0f;
    ...
    for (auto Y = 0; Y < 2 * Camera.GetRenderDistance() + 1; Y++)
    {
        for (auto X = 0; X < 2 * Camera.GetRenderDistance() + 1; X++)
        {
            ...
            {
                WaterAnimIndex0 = ((Ticks() * 3 + ((int)TileX * (int)TileY) * 10) % 3600) / 1200;
            ...
            if (TileX >= 0 && TileY >= 0 && TileX < MapAreaSize && TileY < MapAreaSize)
                Elev0 = Engine.GetCurrentMapArea().Tiles[(int)TileX][(int)TileY].Elevation / ElevAmount - ElevPlayer;

            if (TileX >= 0 && TileY - 1 >= 0 && TileX < MapAreaSize && TileY - 1 < MapAreaSize)
                Elev1 = Engine.GetCurrentMapArea().Tiles[(int)TileX][(int)TileY - 1].Elevation / ElevAmount - ElevPlayer;
            else
                Elev1 = Elev0;

            if (TileX + 1 >= 0 && TileY - 1 >= 0 && TileX + 1 < MapAreaSize && TileY - 1 < MapAreaSize)
                Elev2 = Engine.GetCurrentMapArea().Tiles[(int)TileX + 1][(int)TileY - 1].Elevation / ElevAmount - ElevPlayer;
            else if (TileX + 1 < MapAreaSize)
                Elev2 = Engine.GetCurrentMapArea().Tiles[(int)TileX + 1][(int)TileY].Elevation / ElevAmount - ElevPlayer;
            else if (TileY - 1 >= 0)
                Elev2 = Engine.GetCurrentMapArea().Tiles[(int)TileX][(int)TileY - 1].Elevation / ElevAmount - ElevPlayer;
            else
                Elev2 = Elev0;

            if (TileX + 1 >= 0 && TileY >= 0 && TileX + 1 < MapAreaSize && TileY < MapAreaSize)
                Elev3 = Engine.GetCurrentMapArea().Tiles[(int)TileX + 1][(int)TileY].Elevation / ElevAmount - ElevPlayer;
            else
                Elev3 = Elev0;
            ...
            TileY0 = PlanetTransformator.GetNewY(TileY0, (float)(int)TileX, (float)(int)TileY);
            TileY1 = PlanetTransformator.GetNewY(TileY1, (float)(int)TileX, (float)(int)TileY - 1);
            TileY2 = PlanetTransformator.GetNewY(TileY2, (float)(int)TileX + 1, (float)(int)TileY - 1);
            ...
            for (auto& Object : Engine.GetCurrentMapArea().Tiles[(int)TileX][(int)TileY].Objects)
            {
                auto TileObject = Object->ObjectType;

                if (TileObject != 0)
                    Engine.DrawModel(TileObject,TileX0 + Engine.TileSize / 2, (TileY0 + TileY1 + TileY2 + TileY3)/4.0f, TileZ0 - Engine.TileSize / 2, Object->Rotation, Object->Scaling);
            }

            if (Engine.GetCurrentMapArea().Tiles[(int)TileX][(int)TileY].Mob != nullptr)
            {
                auto SubXPos = (Engine.GetCurrentMapArea().Tiles[(int)TileX][(int)TileY].Mob->Position.X - int(Engine.GetCurrentMapArea().Tiles[(int)TileX][(int)TileY].Mob->Position.X)) * Engine.TileSize;
                auto SubYPos = (Engine.GetCurrentMapArea().Tiles[(int)TileX][(int)TileY].Mob->Position.Y - int(Engine.GetCurrentMapArea().Tiles[(int)TileX][(int)TileY].Mob->Position.Y)) * Engine.TileSize;

                Engine.DrawModel(GetId("MobRabbit"), TileX0 + SubXPos, (TileY0 + TileY1 + TileY2 + TileY3) / 4.0f, TileZ0 - Engine.TileSize + SubYPos, Engine.GetCurrentMapArea().Tiles[(int)TileX][(int)TileY].Mob->FacingAngle);
            }
        }
    }

    auto PlayerX0 = OffsetX + Camera.GetRenderDistance() * Engine.TileSize + Engine.TileSize + SubStepX * Engine.TileSize;
    auto PlayerY0 = 0.0f + Engine.Player.GetModule<cModuleJumping>().PositionZ;
    auto PlayerZ0 = OffsetY + Camera.GetRenderDistance() * Engine.TileSize - Engine.TileSize + SubStepY * Engine.TileSize;

    auto PlayerX1 = OffsetX + Camera.GetRenderDistance() * Engine.TileSize + Engine.TileSize + SubStepX * Engine.TileSize - Engine.TileSize;
    auto PlayerY1 = 0.0f + Engine.Player.GetModule<cModuleJumping>().PositionZ;
    auto PlayerZ1 = OffsetY + Camera.GetRenderDistance() * Engine.TileSize - Engine.TileSize + SubStepY * Engine.TileSize;

    auto PlayerX2 = OffsetX + Camera.GetRenderDistance() * Engine.TileSize + Engine.TileSize + SubStepX * Engine.TileSize - Engine.TileSize;
    auto PlayerY2 = 0.0f + Engine.Player.GetModule<cModuleJumping>().PositionZ;
    auto PlayerZ2 = OffsetY + Camera.GetRenderDistance() * Engine.TileSize - Engine.TileSize + SubStepY * Engine.TileSize + Engine.TileSize;

    auto PlayerX3 = OffsetX + Camera.GetRenderDistance() * Engine.TileSize + Engine.TileSize + SubStepX * Engine.TileSize;
    ...
    Engine.DrawModel(ModelNamePlayer, OffsetX + Camera.GetRenderDistance() * Engine.TileSize + Engine.TileSize / 2.0f + SubStepX * Engine.TileSize, 0.0f + Engine.Player.GetModule<cModuleJumping>().PositionZ, OffsetY + Camera.GetRenderDistance() * Engine.TileSize - Engine.TileSize / 2.0f + SubStepY * Engine.TileSize, Engine.Player.GetModule<cModuleMovement>().FacingAngleRotated);
