...
namespace Forradia
{
    ...
    void ImageLoader::LoadImages()
    {
        ...
        for (auto& file : entries)
        {
            ...
            if (file.is_directory() || imageNameExtension != fullFilename.substr(fullFilename.length() - imageNameExtension.length(), imageNameExtension.length()))
                continue;


            auto surface = SurfacePtr(IMG_Load(file.path().string().c_str()), SDL_Deleter());
